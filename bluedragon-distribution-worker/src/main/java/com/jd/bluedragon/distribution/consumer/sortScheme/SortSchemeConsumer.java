        ...
        if (schemeMq != null && schemeMq.get("target") != null ) {
            ...
            if (stores.contains(target)) {
                ...
                try{
                    ...
                    this.log.debug("[分拣中心分拣方案推送DTC]:接口访问成功，result.getResultCode()={}" , result.getResultCode());
                    this.log.debug("[分拣中心分拣方案推送DTC]:接口访问成功，result.getResultMessage()={}" , result.getResultMessage());
                    this.log.debug("[分拣中心分拣方案推送DTC]:接口访问成功，result.getResultValue()={}" , result.getResultValue());
