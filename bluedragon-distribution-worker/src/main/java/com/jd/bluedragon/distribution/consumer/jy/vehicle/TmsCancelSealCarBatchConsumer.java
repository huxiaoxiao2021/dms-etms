 ...
 * @Auther: 刘铎（liuduo8）
 * @Date: 2022/4/4
 * @Description: 取消封车批次信息 {"sealCarCode":"SC22032919264919","batchCode":"R1508741223106682880","waybillCode":null,"operateUserCode":"xulifan6","operateTime":1648551719281}
 */
...
public class TmsCancelSealCarBatchConsumer extends MessageBaseConsumer {
  ...
  public void consume(Message message) throws Exception {
    ...
    TmsCancelSealCarBatchMQBody mqBody = JsonHelper
        .fromJson(message.getText(), TmsCancelSealCarBatchMQBody.class);
    if (mqBody == null) {
    ...
    if (TMS_CANCEL_SEAL_CAR.equals(sealCarCodeOfTms.getStatus())) {
      ...
      try {
        if (!jyUnSealVehicleService.cancelUnSealTask(convert2Dto(mqBody))) {
          //失败重试
          logger
              .error("TmsCancelSealCarBatchConsumer consume -->关键数据为空，内容为【{}】", message.getText());
        }
      } catch (Exception e) {
        logger.error("jyUnSealVehicleService.cancelUnSealTask error!内容为【{}】", message.getText(), e);
      }
      try {
        sendVehicleTransactionManager
            .resetSendStatusToseal(sealCarCodeOfTms, mqBody.getOperateUserCode(),
                mqBody.getOperateUserName(), mqBody.getOperateTime());
      } catch (Exception e) {
        logger.error("sendVehicleTransactionManager.resetSendStatusToseal error!内容为【{}】",
            message.getText(), e);
      }
    ...
    try {
      ...
      InvokeResult<Boolean> result = jySealVehicleService
          .updateBoardStatusAndSealCode(sealCarCodeOfTms, mqBody.getBatchCode(),
              mqBody.getOperateUserCode(), mqBody.getOperateUserName());
      if (result != null && !result.getData()) {
        logger.error("jySealVehicleService.cancelSealCar 参数为【{}】 error!内容为【{}】", message.getText(),
            result.getMessage());
      }
    } catch (Exception e) {
  ...
  private SealCarDto findSealCarInfoBySealCarCodeOfTms(String sealCarCode) {
    ...
    CommonDto<SealCarDto> sealCarDtoCommonDto = vosManager
        .querySealCarInfoBySealCarCode(sealCarCode);
    if (logger.isInfoEnabled()) {
      logger.info("TmsCancelSealCarBatchConsumer获取封车信息返回数据 {},{}", sealCarCode,
    ...
    if (sealCarDtoCommonDto.getData() == null || StringUtils
        .isEmpty(sealCarDtoCommonDto.getData().getTransWorkItemCode())) {
