...
public class SendDetailConsumer extends MessageBaseConsumer {
    ...
    private void doColdIntercept(Waybill waybill,SendDetailMessage sendDetail){
        ...
        try {
            ...
            if(!isColdProductType){
                log.warn("处理冷链拦截快退逻辑，非冷链卡班、医药大票、冷链小票、医药零担产品类型，无须处理，运单号：{}",waybillCode);
                return;
            }
            ...
            if (SendBizSourceEnum.REVERSE_SEND.getCode().equals(sendDetail.getBizSource())) {
                ...
                if (waybillsign != null && waybillsign.length() > 0) {
                    if (BusinessUtil.isSick(waybill.getWaybillSign())) {
                        this.log.warn("分拣中心逆向病单,冷链卡班、医药大票、冷链小票、医药零担产品屏蔽退款100分MQ,运单号：{}", wayBillCode);
                        return;
                    }
                }
                ...
                this.log.info("冷链卡班、医药大票、冷链小票、医药零担产品 退款100分MQ消息推送成功,运单号：{}", wayBillCode);
            }
        } catch (Exception e) {
            this.log.error("冷链卡班、医药大票、冷链小票、医药零担产品回传退款100分逆向分拣信息失败，运单号：" + wayBillCode, e);
        }
    }
    ...
    private void backwardSendMQ(Waybill waybill,SendDetailMessage sendDetail){
        ...
        if(waybillsign != null && waybillsign.length()>0){
            ...
            if((waybill.getWaybillSign().charAt(0)=='T' || waybill.getWaybillSign().charAt(14)=='6')){
                if(BusinessUtil.isSick(waybill.getWaybillSign())){
                    this.log.warn("分拣中心逆向病单,冷链卡班、医药大票、冷链小票、医药零担产品屏蔽快退MQ,运单号：{}", wayBillCode);
                    return;
                }
                ...
                this.log.info("冷链卡班、医药大票、冷链小票、医药零担产品,分拣中心逆向订单快退:运单号[{}]",wayBillCode);
                try {
                    blockerComOrbrefundRqMQ.send(wayBillCode,json);
                    this.log.info("冷链卡班、医药大票、冷链小票、医药零担产品发送blockerComOrbrefundRq成功,运单号：{}", wayBillCode);
                } catch (Exception e) {
                    this.log.error("冷链卡班、医药大票、冷链小票、医药零担产品,分拣中心逆向订单快退MQ失败[{}]",json , e);
                }
            }else{
    ...
    private FastRefundBlockerComplete toMakeFastRefundBlockerComplete(String waybillCode,SendDetailMessage sendDetail){
        ...
        try{
            ...
            this.log.error("冷链卡班、医药大票、冷链小票、医药零担产品,发送blockerComOrbrefundRq的MQ时新运单号获取老运单号失败,waybillcode:"+waybillCode, e);
        }
