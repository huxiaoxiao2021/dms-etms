...
public class JyUnloadCarAggsMainConsumer extends MessageBaseConsumer {
    ...
    public void consume(Message message) throws Exception {
        ...
        if (!JsonHelper.isJsonString(message.getText())) {
            logger.warn("JyUnloadCarAggsMainConsumer consume -->消息体非JSON格式，内容为【{}】", message.getText());
            return;
        }
        ...
        String lockKey =String.format(CacheKeyConstants.JY_UNLOAD_AGG_LOCK_KEY,entity.getBizId());
        try{
            //加锁处理
            ...
            if(lock){
                ...
                if (redisClientOfJy.exists(versionMutex)) {
                    ...
                    if (!NumberHelper.gt(entity.getVersion(), version)) {
                        logger.warn("JyUnloadCarAggsMainConsumer receive old version data. curVersion: {}, 内容为【{}】", version, message.getText());
                        return;
                    }
                }
                ...
                if(result){
                    ...
                    if (NumberHelper.gt0(entity.getVersion())) {
                        logger.info("JyUnloadCarAggsMainConsumer 卸车汇总消费的最新版本号. {}-{}", entity.getBizId(), entity.getVersion());
                        redisClientOfJy.set(versionMutex, entity.getVersion() + "");
                        redisClientOfJy.expire(versionMutex, 12, TimeUnit.HOURS);
                    }
                }
            }else {
                logger.warn("JyUnloadCarAggsMainConsumer 获取锁失败");
