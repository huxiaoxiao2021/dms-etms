...
public class ShieldsBoxErrorRedisTask extends RedisSingleScheduler {
	@Autowired
	private ShieldsErrorService shieldsErrorService;

	public boolean executeSingleTask(Task task, String ownSign)
			throws Exception {
		return shieldsErrorService.doAddShieldsError(shieldsErrorService
				.doParseShieldsBox(task));
	}
}
