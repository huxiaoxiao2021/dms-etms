 ...
 */
@Service("dmsExpWaybillDeliveryConsumer")
public class DMSExpWaybillDeliveryConsumer extends MessageBaseConsumer {
    private static final Logger logger = LoggerFactory.getLogger(DMSExpWaybillDeliveryConsumer.class);
    ...
    public void consume(Message message) throws Exception {
        ...
        try {
            logger.info("运单妥投消息体-{}",message.getText());
            if (!JsonHelper.isJsonString(message.getText())) {
                logger.warn("运单妥投消息体非JSON格式，内容为【{}】", message.getText());
                return;
            }
            ...
            if(deliveryDto == null || StringUtils.isEmpty(deliveryDto.getWaybillCode())){
                logger.warn("运单妥投消息体异常，内容为【{}】", message.getText());
                return;
            }
            ...
            logger.error("运单妥投消息体处理异常, 消息体:{}", message.getText(), e);
            throw e;
        } finally {
