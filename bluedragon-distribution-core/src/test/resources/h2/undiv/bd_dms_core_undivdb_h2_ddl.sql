...
CREATE TABLE `dms_send_relation` (
	`id` BIGINT ( 20 ) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
	`original_site_code` int(11) NOT NULL DEFAULT 0 COMMENT '始发站点编码',
	`original_site_name` varchar(50) DEFAULT NULL COMMENT '始发站点名称',
	`original_cross_code` varchar(50) DEFAULT NULL COMMENT '始发道口号',
	`destination_site_code` int(11) NOT NULL DEFAULT 0 COMMENT '目的站点编码',
  `destination_site_name` varchar(50) DEFAULT NULL COMMENT '目的站点名称',
	`destination_cross_code` varchar(50) DEFAULT NULL COMMENT '目的道口号',
	`line_type` INT ( 11 ) DEFAULT NULL COMMENT '线路类型:10-干线，20-支线，30-传站，31-长途传站，40-摆渡',	
	`create_time` datetime DEFAULT NULL COMMENT '创建时间',
	`yn` TINYINT ( 1 ) DEFAULT '1' COMMENT '有效标识：1-有效 0-无效',
	`ts` TIMESTAMP ( 3 ) NOT NULL DEFAULT CURRENT_TIMESTAMP ( 3 ) ON UPDATE CURRENT_TIMESTAMP ( 3 ) COMMENT '数据库时间',
	PRIMARY KEY ( `id` ),
	UNIQUE KEY `uniq_dms_send_relation` ( `original_site_code`, `destination_site_code` ) USING BTREE,
	KEY `idx_original_site_code` ( `original_site_code` ),
	KEY `idx_destination_cross_code` ( `destination_cross_code` )  
) ENGINE = INNODB DEFAULT CHARSET = utf8 COMMENT = '分拣发货关系表';
CREATE TABLE `pre_seal_vehicle` (
	`ID` BIGINT ( 20 ) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
	`PRE_SEAL_UUID` VARCHAR ( 80 ) NOT NULL COMMENT '预封车UUID：运力+“-”+时间long',
	`CREATE_SITE_CODE` BIGINT ( 20 ) NOT NULL COMMENT '始发站点Code',
	`CREATE_SITE_NAME` VARCHAR ( 64 ) DEFAULT NULL COMMENT '始发站点名称',
	`RECEIVE_SITE_CODE` BIGINT ( 20 ) NOT NULL COMMENT '目的站点Code',
	`RECEIVE_SITE_NAME` VARCHAR ( 64 ) DEFAULT NULL COMMENT '目的站点名称',
	`TRANSPORT_CODE` VARCHAR ( 64 ) NOT NULL COMMENT '运力编码',
	`VEHICLE_NUMBER` VARCHAR ( 64 ) NOT NULL COMMENT '车牌号',
	`SEAL_CODES` VARCHAR ( 500 ) DEFAULT '' COMMENT '封签号多个逗号分割',
	`SEND_CAR_TIME` VARCHAR ( 20 ) DEFAULT NULL COMMENT '运力最晚发车时间',
	`STATUS` TINYINT ( 1 ) NOT NULL DEFAULT '0' COMMENT '封车状态：0-预封车，1-封车',
	`CREATE_USER_ERP` VARCHAR ( 32 ) NOT NULL COMMENT '创建人ERP',
	`CREATE_USER_NAME` VARCHAR ( 32 ) DEFAULT NULL COMMENT '创建人name',
	`CREATE_TIME` datetime NOT NULL COMMENT '创建时间',
	`UPDATE_USER_ERP` VARCHAR ( 32 ) DEFAULT NULL COMMENT '更新人ERP',
	`UPDATE_USER_NAME` VARCHAR ( 32 ) DEFAULT NULL COMMENT '更新人name',
	`UPDATE_TIME` datetime DEFAULT NULL COMMENT '更新时间',
	`SOURCE` VARCHAR ( 32 ) NOT NULL COMMENT '系统标识',
	`OPERATE_TIME` datetime DEFAULT NULL COMMENT '预封车实操时间',
	`PRE_SEAL_SOURCE` TINYINT ( 1 ) DEFAULT '1' COMMENT '预封车来源：1普通预封车 2传摆预封车',
	`TRANS_WAY` TINYINT ( 1 ) DEFAULT NULL COMMENT '运力类型',
	`TRANS_WAY_NAME` VARCHAR ( 16 ) DEFAULT NULL COMMENT '运力类型名称',
	`WEIGHT` NUMBER ( 8, 3 ) DEFAULT '0.000' COMMENT '重量',
	`VOLUME` NUMBER ( 8, 3 ) DEFAULT '0.000' COMMENT '体积',
	`IS_DELETE` TINYINT ( 1 ) NOT NULL DEFAULT '0' COMMENT '是否删除：0-未删除，1-已删除',
	`TS` TIMESTAMP ( 3 ) NOT NULL DEFAULT CURRENT_TIMESTAMP ( 3 ) ON UPDATE CURRENT_TIMESTAMP ( 3 ) COMMENT '数据库时间',
	PRIMARY KEY ( `ID` ),
	KEY `IDX_PRE_SEAL_SITE_CODE` ( `CREATE_SITE_CODE` ),
	KEY `IDX_PRE_SEAL_TRANSPORT_CODE` ( `TRANSPORT_CODE` ),
	KEY `IDX_VEHICLE_NUMBER` ( `VEHICLE_NUMBER` )
) ENGINE = INNODB DEFAULT CHARSET = utf8 COMMENT = '预封车数据表';
CREATE TABLE `pre_seal_batch` (
  `ID` bigint( 20 ) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `PRE_SEAL_UUID` varchar(80) DEFAULT NULL COMMENT '预封车UUID,对应pre_seal_vehicle表',
  `BATCH_CODE` varchar(50) DEFAULT NULL COMMENT '批次号',
  `CREATE_TIME` datetime DEFAULT NULL COMMENT '创建时间',
  `YN` TINYINT ( 1 ) DEFAULT '1' COMMENT '有效标识：1-有效 0-无效',
  `TS` timestamp( 3 ) NOT NULL DEFAULT CURRENT_TIMESTAMP( 3 ) ON UPDATE CURRENT_TIMESTAMP( 3 ) COMMENT '数据库时间',
  PRIMARY KEY (`ID`),
  KEY `IDX_PRE_SEAL_UUID` (`PRE_SEAL_UUID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='预封车批次数据表';
CREATE TABLE `tms_vehicle_route` (
	`id` BIGINT ( 20 ) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
	`vehicle_job_code` VARCHAR ( 50 ) DEFAULT NULL COMMENT '车次任务编码',
	`vehicle_route_code` VARCHAR ( 50 ) DEFAULT NULL COMMENT '车次任务线路编码',
	`transport_code`  VARCHAR ( 50 ) DEFAULT NULL COMMENT '运力编码',
	`original_site_code` int(11) DEFAULT NULL COMMENT '始发站点编码',
	`destination_site_code` int(11) DEFAULT NULL COMMENT '目的站点编码',
	`depart_time` datetime DEFAULT NULL COMMENT '发车时间',
	`carrier_team_code`  VARCHAR ( 50 ) DEFAULT NULL COMMENT '车队编码',
	`carrier_team_name` VARCHAR ( 50 ) DEFAULT NULL COMMENT '车队名称',
	...
	`ts` TIMESTAMP ( 3 ) NOT NULL DEFAULT CURRENT_TIMESTAMP ( 3 ) ON UPDATE CURRENT_TIMESTAMP ( 3 ) COMMENT '数据库时间',
	PRIMARY KEY ( `id` ),
	UNIQUE KEY `uniq_tms_vehicle_route` ( `vehicle_route_code` ),
	KEY `idx_original_site_code1` ( `original_site_code` ) ,
...
CREATE TABLE `waybill_hasno_presite_record` (
  ...
  `presite_code` int(11) COMMENT '预分拣站点',	
