package com.jd.bluedragon.distribution.test.sorting;
...
public class SortingResourceTestCase {
    ...
    public void is_cancel_waybill() {
        String url = "http://localhost:802/services/waybills/cancel?packageCode={packageCode}";
        Map<String, String> map = new HashMap<String, String>();
        map.put("packageCode", "198000001-1-1-Z");

        WaybillResponse response = this.template.getForObject(url, WaybillResponse.class, map);
    ...
    public void test_sorting_query_packages() {
        String url = "http://localhost:802/services/sorting/check?"
                + "boxCode={boxCode}&createSiteCode={createSiteCode}&"
                + "receiveSiteCode={receiveSiteCode}&businessType={businessType}&packageCode={packageCode}";

        Map<String, String> map = new HashMap<String, String>();
        map.put("boxCode", "W2012041703");
        map.put("createSiteCode", "511");
        map.put("receiveSiteCode", "10091");
        map.put("businessType", "20");
        map.put("packageCode", "W2012041703");

        SortingResponse response = this.template.getForObject(url, SortingResponse.class, map);

        System.out.println("response :: " + response.getCode());
        System.out.println("response :: " + response.getMessage());
    }

    public void test_query_packages_by_boxcode() {
        String url = "http://localhost:80/services/sorting?siteCode={siteCode}&boxCode={boxCode}";

        String[] params = new String[] { "1002", "B1001100500002002" };
        SortingResponse[] response = this.template.getForObject(url, SortingResponse[].class,
                params);
    ...
    public void test_cancel() {
        ...
        sortingRequest.setSiteCode(479);
        sortingRequest.setUserCode(18997);
        sortingRequest.setUserName("陈哲");
        sortingRequest.setPackageCode("218068812-1-1-3");
        sortingRequest.setBusinessType(30);

        ClientRequest request = new ClientRequest(
                "http://dms.360buy.com/services/sorting/cancel");
        request.accept(MediaType.APPLICATION_JSON);

        request.body("application/json", JsonHelper.toJson(sortingRequest));
