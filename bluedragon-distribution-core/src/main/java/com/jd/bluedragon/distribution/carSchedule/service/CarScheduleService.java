...
public interface CarScheduleService {
     ...
     */
    Integer routeTypeByVehicleNoAndSiteCode(String vehicleNo,Integer siteCode);
    ...
    /**
     * 根据车牌号和分拣中心ID获取车辆的包裹总数量（值得注意的是同一辆车中并不是所有的包裹都给到一个目的分拣中心，有可能是两个）
     */
    Integer packageNumByVehicleNoAndSiteCode(String vehicleNo,Integer siteCode);

    /**
     * 根据车牌号获取当前分拣中心本次车辆任务的载货量明细
     */
    List<SendDetail> sendDetailByCarAndSiteCode(String vehicleNo , Integer siteCode);

    /**
     * 根据车牌号获取当前分拣中心的运输包裹数量
     */
