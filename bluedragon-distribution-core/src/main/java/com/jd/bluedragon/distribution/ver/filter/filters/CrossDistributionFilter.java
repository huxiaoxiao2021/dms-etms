...
public class CrossDistributionFilter implements Filter {
    ...
    public void doFilter(FilterContext request, FilterChain chain) throws Exception {
        logger.info("分拣校验CrossDistributionFilter1packageCode[{}]pdaOperateRequest[{}]rule[{}]",request.getPackageCode(), JsonHelper.toJson(request), JsonHelper.toJson(request));
        //大件订单 不进行预分拣相关校验 tangcq2018年11月2日10:28:42
        if (BusinessHelper.isLasOrder(request.getWaybillCache().getWaybillSign())){
            chain.doFilter(request, chain);
            return;
        }
        logger.info("分拣校验CrossDistributionFilter2packageCode[{}]pdaOperateRequest[{}]rule[{}]",request.getPackageCode(), JsonHelper.toJson(request), JsonHelper.toJson(request));
        /* 填航空仓不进行目的地校验 */
        ...
        logger.info("分拣校验CrossDistributionFilter3packageCode[{}]pdaOperateRequest[{}]rule[{}]",request.getPackageCode(), JsonHelper.toJson(request), JsonHelper.toJson(request));
        ...
        logger.info("分拣校验CrossDistributionFilter4packageCode[{}]pdaOperateRequest[{}]rule[{}]",request.getPackageCode(), JsonHelper.toJson(request), JsonHelper.toJson(request));
...
//            Site waybillSite = waybill.getSiteCode()!=null&&waybill.getSiteCode().intValue()>0?siteService.get(waybill.getSiteCode()):null;
            ...
            if (SiteHelper.isDistributionCenter(request.getReceiveSite())
                ...
                if (rule6 == null || !SWITCH_ON.equals(rule6.getContent())) {
                    logger.info("分拣校验CrossDistributionFilter7packageCode[{}]pdaOperateRequest[{}]rule[{}]",request.getPackageCode(), JsonHelper.toJson(request), JsonHelper.toJson(request));
