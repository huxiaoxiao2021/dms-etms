...
public class JyUnloadVehicleTysServiceImpl implements JyUnloadVehicleTysService {
    ...
    private Date waitUnloadQueryTimeRange() {
        return DateHelper.getZeroFromDay(new Date(), dmsConfigManager.getUccPropertyConfig().getJyUnloadCarListQueryDayFilter());
    }
    ...
    private Date doneUnloadQueryTimeRange() {
        return DateHelper.getZeroFromDay(new Date(), dmsConfigManager.getUccPropertyConfig().getJyUnloadCarListDoneQueryDayFilter());
    }
...
//    }
    ...
    private void setAutoRefreshConfig(UnloadVehicleTaskRespDto respDto){
        // 增加刷新间隔配置
        try {
            final ClientAutoRefreshConfig jyWorkAppAutoRefreshConfig = dmsConfigManager.getUccPropertyConfig().getJyWorkAppAutoRefreshConfigByBusinessType(ClientAutoRefreshBusinessTypeEnum.TYS_UNLOAD_TASK_LIST.name());
...
//    }
    ...
    public InvokeResult<ScanPackageRespDto> scan(ScanPackageDto scanPackageDto) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(scanPackageDto.getSealCarCode())) {
                if (dmsConfigManager.getUccPropertyConfig().isPdaVersionSwitch()) {
                    // 新老版本互斥
    ...
    private InvokeResult<ScanPackageRespDto> packageScan(ScanPackageDto scanPackageDto, JyBizTaskUnloadVehicleEntity unloadVehicleEntity,
        ...
        if (packageD == null) {
            ...
            if(dmsConfigManager.getUccPropertyConfig().getWaybillSysNonExistPackageInterceptSwitch()) {
        ...
        //德邦单号场地转发提醒
        if (dmsConfigManager.getUccPropertyConfig().isDpTransferSwitch()) {
        ...
        //易冻品校验
        if (dmsConfigManager.getUccPropertyConfig().isEasyFreezeSwitch()) {
...
//            logEngine.addLog(businessLogProfiler);
    ...
    private InvokeResult<Void> scanSupplementCheck(JyBizTaskUnloadVehicleEntity unloadVehicleEntity) {
        ...
        if(JyBizTaskUnloadStatusEnum.UN_LOAD_DONE.getCode().equals(unloadVehicleEntity.getVehicleStatus())) {
            ...
            Long limitTime = dmsConfigManager.getUccPropertyConfig().getTysUnloadTaskSupplementScanLimitHours() * 3600l * 1000l;
            if(log.isInfoEnabled()) {
                log.info("JyUnloadVehicleTysServiceImpl.scanSupplementCheck--扫描补扫校验--任务信息={}，{}小时后不可补扫",
                        JsonUtils.toJSONString(unloadVehicleEntity), JsonUtils.toJSONString(unloadVehicleEntity), dmsConfigManager.getUccPropertyConfig().getTysUnloadTaskSupplementScanLimitHours());
            }
            if(System.currentTimeMillis() - completeTime - limitTime > 0) {
                String msg = String.format("该任务已结束%s小时，禁止补扫，可自建任务扫描", dmsConfigManager.getUccPropertyConfig().getTysUnloadTaskSupplementScanLimitHours());
    ...
    public InvokeResult<Void> handoverTask(UnloadVehicleTaskDto unloadVehicleTask) {
        ...
        try {
            ...
            if(normalTaskNum >= dmsConfigManager.getUccPropertyConfig().getTysUnloadTaskHandoverMaxSize() + 1) {
                if(log.isInfoEnabled()) {
                    log.info("JyUnloadVehicleTysServiceImpl.handoverTask--限制交接班此时，当前查到子任务数为{}，限制最大交接次数为{}，请求={}",
                            normalTaskNum, dmsConfigManager.getUccPropertyConfig().getTysUnloadTaskHandoverMaxSize(), JsonHelper.toJson(unloadVehicleTask));
                }
