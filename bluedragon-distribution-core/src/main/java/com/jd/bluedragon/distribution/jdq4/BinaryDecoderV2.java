...
public class BinaryDecoderV2 extends Decoder {
    public static final int DEFAULT_BUFFER_SIZE = 8192;
    private BinaryDecoderV2.ByteSource source = null;
    ...
    protected BinaryDecoderV2() {
    }

    public BinaryDecoderV2(InputStream in, int bufferSize) {
        this.configure(in, bufferSize);
    }

    public BinaryDecoderV2(byte[] data, int offset, int length) {
        this.configure(data, offset, length);
    }

    public BinaryDecoderV2 configure(InputStream in, int bufferSize) {
        this.configureSource(bufferSize, new BinaryDecoderV2.InputStreamByteSource(in));
        return this;
    }

    public BinaryDecoderV2 configure(byte[] data, int offset, int length) {
        this.configureSource(8192, new BinaryDecoderV2.ByteArrayByteSource(data, offset, length));
        return this;
    }

    private void configureSource(int bufferSize, BinaryDecoderV2.ByteSource source) {
        if (null != this.source) {
            this.source.detach();
        }
    ...
    public String readString() throws IOException {
        ...
        if (length < 0) {
            ...
            if (length <= remaining) {
                result = new String(this.buf, this.pos, length);
                this.pos += length;
            } else {
                result = new String(this.buf, this.pos, remaining);
                start = start + remaining;
                length -= remaining;
                this.pos = this.limit;
                this.source.readRaw((byte[])null, start, length);
            }
    ...
    private static class ByteArrayByteSource extends BinaryDecoderV2.ByteSource {
        private byte[] data;
        ...
        protected void attach(int bufferSize, BinaryDecoderV2 decoder) {
            decoder.buf = this.data;
            decoder.pos = this.position;
            decoder.minPos = this.position;
            decoder.limit = this.max;
            this.ba = new BinaryDecoderV2.BufferAccessor(decoder);
        }
    ...
    private static class InputStreamByteSource extends BinaryDecoderV2.ByteSource {
    ...
    abstract static class ByteSource extends InputStream {
        protected BinaryDecoderV2.BufferAccessor ba;
    ...
    static class BufferAccessor {
        private final BinaryDecoderV2 decoder;
        private byte[] buf;
        private int pos;
        private int limit;
        boolean detached;

        private BufferAccessor(BinaryDecoderV2 decoder) {
            this.detached = false;
