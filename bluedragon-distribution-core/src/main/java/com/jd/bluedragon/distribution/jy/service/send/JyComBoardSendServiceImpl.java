...
public class JyComBoardSendServiceImpl implements JyComBoardSendService {
  ...
  public InvokeResult<TableTrolleyResp> listTableTrolleyUnderCross(TableTrolleyReq request) {
    ...
    if (ObjectHelper.isNotNull(dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit()) && dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit()>0){
      tableTrolleyResp.setSendFlowCountLimitUnderCtt(dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit());
    }
    ...
    if (!StringUtils.isEmpty(request.getCrossCode())) {
      ...
      // 根据场地查询笼车信息
      query.setLimit(dmsConfigManager.getUccPropertyConfig().getJyComboardSiteCTTPageSize());
    ...
    if (tableTrolleyJsfResp != null && !CollectionUtils
      ...
      if (request.getNeedSendFlowStatistics() && ObjectHelper.isNotNull(request.getCrossCode())) {
        ...
        try {
          ...
          Date queryTime = DateHelper.addHoursByDay(new Date(), -Double.valueOf(dmsConfigManager.getUccPropertyConfig().getJyComboardTaskCreateTimeBeginDay()));
  ...
  public InvokeResult<CreateGroupCTTResp> createGroupCTTData(CreateGroupCTTReq request) {
    ...
    if (request.getTableTrolleyDtoList().size() > dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit()) {
      throw new JyBizException("混扫任务流向不能超过"+ dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit()+"个！");
    }
  ...
  public InvokeResult addCTT2Group(AddCTTReq request) {
    ...
    if (!CollectionUtils.isEmpty(sendFlowList)) {
      ...
      if (sendFlowSize > dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit()) {
        return new InvokeResult( UPDATE_CTT_GROUP_LIST_CODE, "混扫任务流向不能超过"+ dmsConfigManager.getUccPropertyConfig().getCttGroupSendFLowLimit()+"个");
      }
    }
  ...
  public InvokeResult<SendFlowDataResp> listSendFlowUnderCTTGroup(SendFlowDataReq request) {
    ...
    Date time = DateHelper.addDate(DateHelper.getCurrentDayWithOutTimes(), -dmsConfigManager.getUccPropertyConfig().getJyComboardScanUserBeginDay());
    ...
    //查询多个流向下n天内未封车的板数量
    BoardCountReq boardCountReq = new BoardCountReq();
    Date queryTime = DateHelper.addDate(DateHelper.getCurrentDayWithOutTimes(), -dmsConfigManager.getUccPropertyConfig().getJyComboardTaskCreateTimeBeginDay());
    boardCountReq.setCreateTime(queryTime);
  ...
  private void getSendFlowDtoList(List<JyGroupSortCrossDetailEntity> sendFlowList,
    ...
    for (JyGroupSortCrossDetailEntity entity : sendFlowList) {
      ...
      if (boardFlow != null) {
        ...
        boardDto.setBoardScanLimit(dmsConfigManager.getUccPropertyConfig().getJyComboardCountLimit());
        // 已扫比例
        int scanCount = 0;
        ...
        int scanProgress = (int) ((scanCount * 1.00 / dmsConfigManager.getUccPropertyConfig().getJyComboardCountLimit()) * 100);
  ...
  public JdVerifyResponse<ComboardScanResp> comboardScan(ComboardScanReq request) {
    ...
    ComboardScanResp resp = assembleComboardResp(request);
    if (dmsConfigManager.getUccPropertyConfig().getSupportMutilScan() && request.getNeedSkipSendFlowCheck()){
  ...
  private boolean checkIntercept(ComboardScanReq request) {
    if ((dmsConfigManager.getUccPropertyConfig().getInterceptBlackList().equals(Constants.TOTAL_URL_INTERCEPTOR) || checkContainsCurrentSite(request)) && request.getNeedIntercept()){
      return true;
    }
    return false;
  }
  ...
  private boolean checkContainsCurrentSite(ComboardScanReq request) {
    if (ObjectHelper.isNotNull(request.getCurrentOperate().getSiteCode())){
      ...
      if (dmsConfigManager.getUccPropertyConfig().getInterceptBlackList().contains(Constants.SEPARATOR_COMMA)){
        siteList =Arrays.asList(dmsConfigManager.getUccPropertyConfig().getInterceptBlackList().split(Constants.SEPARATOR_COMMA));
      }
      else {
        siteList.add(dmsConfigManager.getUccPropertyConfig().getInterceptBlackList());
      }
  ...
  private void execComboard(ComboardScanReq request) {
    ...
    try {
      ...
      if (!entity.getBulkFlag() && entity.getHaveScanCount() < dmsConfigManager.getUccPropertyConfig().getJyComboardCountLimit()) {
        Date now = new Date();
  ...
  private void getOrCreateBoardCode(ComboardScanReq request) {
    ...
    if (dmsConfigManager.getUccPropertyConfig().getCreateBoardBySendFlowSwitch() && !jimDbLock.lock(sendFlowLockKey, request.getRequestId(), LOCK_EXPIRE, TimeUnit.SECONDS)) {
      throw new JyBizException("当前系统繁忙,请稍后再试！");
    }
    ...
    if (!dmsConfigManager.getUccPropertyConfig().getCreateBoardBySendFlowSwitch() && !jimDbLock.lock(sendFlowAndGroupLockKey, request.getRequestId(), LOCK_EXPIRE, TimeUnit.SECONDS)) {
      throw new JyBizException("当前系统繁忙,请稍后再试！");
    }
    try {
      //查询当前流向是否存在进行中的板
      ...
      if (ObjectHelper.isNotNull(boardDto)) {
        ...
        if (!boardDto.getBulkFlag() && boardDto.getCount() < dmsConfigManager.getUccPropertyConfig().getJyComboardCountLimit()) {
  ...
  private void comboardCheck(ComboardScanReq request) {
    ...
    if (WaybillUtil.isPackageCode(barCode) || WaybillUtil.isWaybillCode(barCode)) {
      ...
      if (WaybillUtil.isWaybillCode(barCode) && !WaybillUtil.isPackageCode(request.getBarCode()) && waybill.getGoodNumber() < dmsConfigManager.getUccPropertyConfig().getBulkScanPackageMinCount()) {
    ...
    if (ObjectHelper.isNotNull(entity)) {
      ...
      if (comboardTime != null && System.currentTimeMillis() - comboardTime.getTime() <=  dmsConfigManager.getUccPropertyConfig().getReComboardTimeLimit() * 3600L * 1000L) {
    ...
    if (recentSendMByParam != null) {
      ...
      if (sendTime != null
          && System.currentTimeMillis() - sendTime.getTime() <= dmsConfigManager.getUccPropertyConfig().getReComboardTimeLimit() * 3600L * 1000L) {
  ...
  private boolean checkReComboard(Sorting sorting) {
    if (ObjectHelper.isNotNull(sorting)){
      ...
      if (sortingCreateTime != null && System.currentTimeMillis() - sortingCreateTime.getTime() >  dmsConfigManager.getUccPropertyConfig().getReComboardTimeLimit() * 3600L * 1000L) {
        return true;
      }
    }
    return false;
  ...
  boolean needForceSend(Integer siteId){
    if (dmsConfigManager.getUccPropertyConfig().getForceSendSiteList().equals(Constants.TOTAL_URL_INTERCEPTOR) || dmsConfigManager.getUccPropertyConfig().getForceSendSiteList().contains(String.valueOf(siteId))){
      return true;
    }
    return false;
  }
  ...
  private void matchDestinationCheck(ComboardScanReq request) {
    ...
    if (!request.getEndSiteId().equals(request.getDestinationId()) && !request.getNeedSkipSendFlowCheck()){
      if (dmsConfigManager.getUccPropertyConfig().getSupportMutilScan()){
  ...
  private SendFlowDto assemblySendFlowParams(BoardStatisticsReq request) {
    ...
    sendFlowDto.setQueryTimeBegin(DateHelper.addHoursByDay(new Date(), -Double.valueOf(dmsConfigManager.getUccPropertyConfig().getJyComboardTaskCreateTimeBeginDay())));
    sendFlowDto.setQuerySealTimeBegin(DateHelper.addHoursByDay(new Date(), -Double.valueOf(dmsConfigManager.getUccPropertyConfig().getJyComboardTaskSealTimeBeginDay())));
    List<Integer> comboardSourceList = new ArrayList<>();
    comboardSourceList.add(JyBizTaskComboardSourceEnum.ARTIFICIAL.getCode());
    sendFlowDto.setComboardSourceList(comboardSourceList);
    return sendFlowDto;
  ...
  private boolean checkSealTime(JyBizTaskComboardEntity comboardEntity) {
    ...
    if ( System.currentTimeMillis() - comboardEntity.getSealTime().getTime() >  dmsConfigManager.getUccPropertyConfig().getReComboardTimeLimit() * 3600L * 1000L) {
      return false;
    }
    return true;
  }
  ...
  public InvokeResult<BoardQueryResp> listComboardBySendFlow(BoardQueryReq request) {
    ...
    boardQueryResp.setBoardLimit(dmsConfigManager.getUccPropertyConfig().getJyComboardSealBoardListLimit());
    boardQueryResp.setBoardSelectLimit(dmsConfigManager.getUccPropertyConfig().getJyComboardSealBoardListSelectLimit());

    // 获取当前场地未封车的板号
    SendFlowDto sendFlow = new SendFlowDto();
    sendFlow.setEndSiteId(request.getEndSiteId());
    sendFlow.setStartSiteId(request.getCurrentOperate().getSiteCode());
    Date time = DateHelper.addHoursByDay(new Date(), -Double.valueOf(dmsConfigManager.getUccPropertyConfig().getJyComboardSealQueryBoardListTime()));
    log.info("组板列表查询创建时间ucc配置：{}", dmsConfigManager.getUccPropertyConfig().getJyComboardSealQueryBoardListTime());
  ...
  private boolean needIsolateBoardByGroupCode(CurrentOperate currentOperate) {
    if (dmsConfigManager.getUccPropertyConfig().getNeedIsolateBoardByGroupCodeSiteList().contains(String.valueOf(currentOperate.getSiteCode()))){
      return true;
    }
    return false;
  }
  ...
  public InvokeResult<SendFlowDataResp> queryScanUser(SendFlowQueryReq request) {
    ...
    resp.setScanUserList(userList);
    resp.setTimerInterval(dmsConfigManager.getUccPropertyConfig().getJyComboardRefreshTimerInterval());
    return new InvokeResult<>(RESULT_SUCCESS_CODE,RESULT_SUCCESS_MESSAGE,resp);
  }
