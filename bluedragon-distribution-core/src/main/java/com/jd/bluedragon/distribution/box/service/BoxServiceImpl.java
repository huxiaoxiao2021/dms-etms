...
public class BoxServiceImpl implements BoxService {
     ...
     */
    @Override
    @JProfiler(jKey = "DMSWEB.BoxService.createBox",jAppName = Constants.UMP_APP_NAME_DMSWEB, mState = {JProEnum.TP})
    public Result<CreateBoxInfo> createBox(CreateBoxReq createBoxReq) {
        if(log.isInfoEnabled()){
            log.info("BoxServiceImpl.createBox param {}", JsonHelper.toJson(createBoxReq));
        }
        ...
        try {
            ...
            final Result<Void> checkResult = this.checkParam4CreateBox(createBoxReq);
            if (checkResult.isFail()) {
                return result.toFail(checkResult.getMessage(), checkResult.getCode());
            }
            ...
            List<Box> availableBoxes;
            availableBoxes = this.batchAddNew(this.toBox(createBoxReq, createSiteInfo), BoxSystemTypeEnum.AUTO_SORTING_MACHINE.getCode());
    ...
    private Result<Void> checkParam4CreateBox(CreateBoxReq createBoxReq) {
        Result<Void> checkResult = Result.success();
