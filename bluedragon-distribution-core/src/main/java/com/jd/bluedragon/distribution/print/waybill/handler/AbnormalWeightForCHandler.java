...
public class AbnormalWeightForCHandler implements Handler<WaybillPrintContext,JdResult<String>>{
				...
				if (request.hasWeighted()) {
					...
					if (weight > thresholdLow && weight <= thresholdHigh) {
						if (!Objects.equals(Boolean.TRUE, request.getConfirm())) {
							context.getResult().toWeakSuccess(JdResponse.CODE_PRINT_WEIGHT_WARNING, JdResponse.MESSAGE_CODE_PRINT_WEIGHT_WARNING);
						}
					}
					// 强制拦截状态，不通过
