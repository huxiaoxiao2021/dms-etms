...
public class SimpleScannerFrameDispatchServiceImpl implements ScannerFrameDispatchService {
    ...
    public boolean dispatch(UploadData domain) {
        ...
        // 判断操作类型是否为发货 此处需要添加判断是否为新龙门架
        if (config.getBusinessType() == 2) {
    ...
    private String getSendCodeWithBoxCode(UploadData domain, GantryDeviceConfig config) {
        ...
        if (box != null) {
            BaseStaffSiteOrgDto baseStaffSiteOrgDto = siteService.getSite(box.getReceiveSiteCode());
            if (baseStaffSiteOrgDto != null) {
                ...
                if (logger.isWarnEnabled()) {
                    logger.warn(MessageFormat.format("龙门架自动发货,根据箱号获取批次号registerNo={0},operateTime={1},siteCode={2}|目的站点信息查询结果为NULL", domain.getRegisterNo(), domain.getScannerTime(), box.getReceiveSiteCode()));
                }
            }
        } else {
            if (logger.isWarnEnabled()) {
