...
public abstract class AbstractExcelDataResolver implements DataResolver{
	...
	@Override
	public <T> List<T> resolverExcel(InputStream in, Class<T> cls, MetaDataFactory metaDataFactory,boolean validateIsContinue,List<String> resultMessages) throws Exception{
		List<T> ret = new ArrayList<T>();
		long startTime = System.currentTimeMillis();
		Workbook workbook = this.createWorkbook(in);
		int sheets = workbook.getNumberOfSheets();
		...
		if (null != sheet && sheet.getLastRowNum() > excelConfig.getMaxNumber() + 1) {
			...
			throw new IllegalArgumentException("导入数据超出1000条");
		}
