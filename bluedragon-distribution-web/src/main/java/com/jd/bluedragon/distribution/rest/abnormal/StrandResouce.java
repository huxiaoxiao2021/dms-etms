...
public class StrandResouce {
    ...
    @Autowired
    private ConfigStrandReasonServiceJsfManager configStrandReasonServiceJsfManager;    
    ...
    public InvokeResult<Boolean> report(StrandReportRequest request){
        ...
        try {
        	...
        	Result<ConfigStrandReason> reasonInfo = configStrandReasonServiceJsfManager.queryByReasonCode(request.getReasonCode());
        	if(reasonInfo == null
    ...
    public InvokeResult<List<ConfigStrandReasonData>> queryReasonList(){
        ...
        try {
        	...
        	Result<PageData<ConfigStrandReasonVo>> resultInfo = configStrandReasonServiceJsfManager.queryPageList(query);
        	if(resultInfo == null
        	...
        	invokeResult.setData(toConfigStrandReasonDataList(resultInfo.getData().getRecords()));
        }catch (Exception e){
     ...
     * @return
     */
	private List<ConfigStrandReasonData> toConfigStrandReasonDataList(List<ConfigStrandReasonVo> records) {
		List<ConfigStrandReasonData> list = new ArrayList<>();
		if(!CollectionUtils.isEmpty(records)) {
			for(ConfigStrandReasonVo vo : records) {
