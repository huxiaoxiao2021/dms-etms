...
public class ClientAuthResource {
    ...
    public InvokeResult<List<Integer>> getNoAuthMenuCode(ClientAuthRequest clientAuthRequest){
     ...
     * @return
     */
    private List<Integer> obtainNoAuthMenu(BaseStaffSiteOrgDto baseSite) {
        List<Integer> noAuthMenuCodeList = new ArrayList<Integer>();
        Integer siteType = baseSite.getSiteType();
        Integer subType = baseSite.getSubType();
        // 打印客户端无权限的菜单编码
        String noAuthMenuConfig = uccPropertyConfiguration.getNoAuthMenuConfig();
    ...
    public InvokeResult<ClientMenuDto> getMenuAuthFunc(ClientAuthRequest clientAuthRequest){
        ...
        clientMenuDto.setMenuName(ClientMenuEnum.getEnumName(clientAuthRequest.getMenuCode()));
        // 判断菜单是否拥有某功能
    ...
    private void checkMenuFuncAuth(ClientAuthRequest clientAuthRequest, ClientMenuDto clientMenuDto) {
        ...
        String menuCodeFuncConfig = uccPropertyConfiguration.getMenuCodeFuncConfig();
