...
public class JdCloudPrintServiceTestCase {
    ...
    private static void testJdClodUrl() {
		...
		log.info(MediaType.APPLICATION_JSON.getCharSet().toString());
		...
		log.info("开始调用云打印,req:{}", JsonHelper.toJson(formEntity));
		log.info("开始调用云打印,req:{}",req+type.getCharSet());
		...
		log.info("开始调用云打印,resp:{}", JsonHelper.toJson(responseEntity));
	}
